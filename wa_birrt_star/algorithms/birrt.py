...
def try_connect_tree(fw_tree, rv_tree, sigma, obstacles):
    ...
    returns:      (forward_node, reverse_node) if connected
                  (None, None) if not connected
    """
...
def merge_path(forward_node, reverse_node):
    ...
    """
    # Path from start → forward connection point
    forward_path = extract_path(forward_node)

    # Path from goal → reverse connection point, then flip
    reverse_path = extract_path(reverse_node)
    reverse_path.reverse()   # now goes connection → goal

    # Calculate midpoint between the two connection nodes
    # This bridges the small gap between them
    fx, fy = forward_node["pos"]
    rx, ry = reverse_node["pos"]
    midpoint = ((fx + rx) / 2, (fy + ry) / 2)

    # Stitch: forward path + midpoint bridge + reverse path
...
def birrt(start, goal, obstacles, map_size,
          max_iter=5000,
          step_size=0.15,
          sigma=0.5,        # smaller connection threshold
          goal_bias=0.10):
    """
