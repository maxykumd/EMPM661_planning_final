...
def is_collision(pos, obstacles, clearance=None):
    ...
    clearance: override the default CLEARANCE (used when obstacles are pre-inflated)
    """
    if clearance is None:
        clearance = CLEARANCE   # default — uses mock_data value
...
def is_collision_free_path(pos_a, pos_b, obstacles, clearance=None):
    ...
    Uses adaptive num_checks based on segment length so no obstacle
    can slip between check points regardless of segment length.
    """
    if clearance is None:
        clearance = CLEARANCE
    ...
    # Step size = half the smallest obstacle clearance zone
    # guarantees no obstacle (min radius ~0.20m) slips between checks
    check_step  = (CLEARANCE) / 2.0      # 0.10m steps
